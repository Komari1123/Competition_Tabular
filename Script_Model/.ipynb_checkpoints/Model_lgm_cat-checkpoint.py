...
def model_lgm(train_df , target , params , cat_list, cv_ope , cv_num):
    score_list = [] 
    models = []
    ...
    for fold_, (train_index, valid_index) in tqdm(enumerate(kf.split(train_df,target))): 
        ...
        lgb_train= lgb.Dataset(train_x , train_y , categorical_feature = cat_list)
        lgb_valid = lgb.Dataset(valid_x , valid_y , categorical_feature = cat_list)
    ...
    print('平均 : R^2 ', round(np.mean(score_list),5))
    return models

def mdoel_lgm_importance(df,models):
    fe_importance = np.zeros((len(df.columns), 10))
    ...
    importance = np.concatenate([importance_mean,importance_std], axis = 1)
    
    importance = pd.DataFrame(importance, index=train_df.columns, columns=['importance_mean','importance_std']
                             ).sort_values('importance_mean', ascending=False)
    
    print(importance.sort_values('importance', ascending=True))
    
#     plt.figure(figsize=(7, 10))
    plt.barh(importance.index,importance.importance_mean)
    plt.barh(importance.index,importance.importance_std)
